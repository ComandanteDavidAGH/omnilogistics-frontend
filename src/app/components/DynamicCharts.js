...
export default function DynamicCharts({ columns, data, yearA, yearB }) {
  ...
  const exportFormattedExcel = () => {
    ...
    <Styles>
      <Style ss:ID="TitleHdr"><Alignment ss:Horizontal="Center" ss:Vertical="Center"/><Font ss:FontName="Segoe UI" ss:Size="14" ss:Bold="1" ss:Color="#FFFFFF"/><Interior ss:Color="#0F172A" ss:Pattern="Solid"/></Style>
      <Style ss:ID="TableHdr"><Alignment ss:Horizontal="Center" ss:Vertical="Center"/><Font ss:FontName="Segoe UI" ss:Size="10" ss:Bold="1" ss:Color="#FFFFFF"/><Interior ss:Color="#1E3A8A" ss:Pattern="Solid"/><Borders><Border ss:Position="Bottom" ss:LineStyle="Continuous" ss:Weight="1" ss:Color="#64748B"/></Borders></Style>
      <Style ss:ID="TableHdrLeft"><Alignment ss:Horizontal="Left" ss:Vertical="Center" ss:Indent="1"/><Font ss:FontName="Segoe UI" ss:Size="10" ss:Bold="1" ss:Color="#FFFFFF"/><Interior ss:Color="#1E3A8A" ss:Pattern="Solid"/></Style>
      <Style ss:ID="CellData"><Alignment ss:Horizontal="Center" ss:Vertical="Center"/><Font ss:FontName="Segoe UI" ss:Size="10"/><Borders><Border ss:Position="Bottom" ss:LineStyle="Continuous" ss:Weight="1" ss:Color="#CBD5E1"/></Borders></Style>
      <Style ss:ID="CellDataLeft"><Alignment ss:Horizontal="Left" ss:Vertical="Center" ss:Indent="1"/><Font ss:FontName="Segoe UI" ss:Size="10" ss:Bold="1" ss:Color="#0F172A"/><Borders><Border ss:Position="Bottom" ss:LineStyle="Continuous" ss:Weight="1" ss:Color="#CBD5E1"/></Borders></Style>
    </Styles>`;
    ...
    if (isUltra) {
      ...
      ultraMetrics.forEach(metric => {
        ...
        xml += `<Row ss:Height="30"><Cell ss:MergeAcross="3" ss:StyleID="TitleHdr"><Data ss:Type="String">INFORME EJECUTIVO: ${esc(metric).toUpperCase()}</Data></Cell></Row>`;
        xml += `<Row ss:Height="24">
            <Cell ss:StyleID="TableHdrLeft"><Data ss:Type="String">SEMANA - CINTA</Data></Cell>
            <Cell ss:StyleID="TableHdr"><Data ss:Type="String">${esc(metric)} (${yearA})</Data></Cell>
            <Cell ss:StyleID="TableHdr"><Data ss:Type="String">${esc(metric)} (${yearB})</Data></Cell>
            <Cell ss:StyleID="TableHdr"><Data ss:Type="String">VARIACIÓN (B-A)</Data></Cell>
          </Row>`;
        chartData.forEach(row => {
          const vA = row[`${metric}_${yearA}`] || 0; const vB = row[`${metric}_${yearB}`] || 0;
          xml += `<Row><Cell ss:StyleID="CellDataLeft"><Data ss:Type="String">${esc(row.name)}</Data></Cell><Cell ss:StyleID="CellData"><Data ss:Type="Number">${vA}</Data></Cell><Cell ss:StyleID="CellData"><Data ss:Type="Number">${vB}</Data></Cell><Cell ss:StyleID="CellData"><Data ss:Type="Number">${Number((vB-vA).toFixed(2))}</Data></Cell></Row>`;
        });
        xml += `</Table></Worksheet>`;
      });
    } else {
      ...
      xml += `</Row>`;
      chartData.forEach(row => {
        xml += `<Row><Cell ss:StyleID="CellDataLeft"><Data ss:Type="String">${esc(row.name)}</Data></Cell>`;
        numCols.forEach(c => xml += `<Cell ss:StyleID="CellData"><Data ss:Type="Number">${row[c] || 0}</Data></Cell>`);
        xml += `</Row>`;
      });
      xml += `</Table></Worksheet>`;
    }
    ...
    const link = document.createElement('a'); link.href = url; link.download = `Reporte_Omnilogistics.xls`;
    ...
    <div className="bg-slate-900 border border-slate-800 p-6 rounded-xl shadow-2xl my-6 space-y-6 print:bg-white print:text-black print:border-none print:shadow-none">
      ...
      <div className="bg-indigo-950/30 border border-indigo-500/50 rounded-xl p-5 print:bg-slate-50 print:border-slate-300">
        ...
        <ul className="space-y-1">
          {rawInsights.map((ins, i) => <li key={i} className="text-xs text-slate-300 list-disc ml-5 print:text-slate-800">{ins}</li>)}
        </ul>
      </div>
        ...
        {isUltra && (
          <div className="flex overflow-x-auto bg-slate-900 border-b border-slate-800 print:hidden custom-scrollbar">
            {ultraMetrics.map((metric, idx) => (
              <button key={idx} onClick={() => setActiveTab(metric)} className={`whitespace-nowrap px-4 py-3 text-xs font-bold border-b-2 ${activeTab === metric ? 'border-blue-500 text-blue-400 bg-slate-800/50' : 'border-transparent text-slate-400 hover:text-slate-200'}`}>
                {metric.toUpperCase()}
              </button>
            ))}
          </div>
              ...
              <tr>
                ...
                {isUltra ? (
                  <>
                    ...
                    <th className="px-4 py-3 border-b border-slate-700 text-emerald-400">{activeTab} ({yearB})</th>
                    <th className="px-4 py-3 border-b border-slate-700 text-slate-200">Variación</th>
                  </>
                ) : (
            ...
            <tbody>
              {chartData.map((row, i) => (
                <tr key={i} className="border-b border-slate-800/50 hover:bg-slate-800/30">
                  ...
                  {isUltra ? (
                    <>
                      <td className="px-4 py-1.5">{row[`${activeTab}_${yearA}`] || 0}</td>
                      <td className="px-4 py-1.5 text-emerald-300">{row[`${activeTab}_${yearB}`] || 0}</td>
                      <td className="px-4 py-1.5 font-bold">
                        {(() => {
                          const diff = (row[`${activeTab}_${yearB}`] || 0) - (row[`${activeTab}_${yearA}`] || 0);
                          return <span className={diff > 0 ? 'text-emerald-500' : diff < 0 ? 'text-red-500' : 'text-slate-500'}>{diff > 0 ? '+' : ''}{diff.toFixed(2)}</span>;
                        })()}
                      </td>
                    </>
                  ) : (
